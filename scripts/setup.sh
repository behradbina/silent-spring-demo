...
case "$OS" in Darwin) PLAT=darwin ;; Linux) PLAT=linux ;; *) echo "unsupported OS $OS"; exit 1 ;; esac
case "$ARCH" in arm64|aarch64) NARCH=arm64 ;; x86_64) NARCH=x64 ;; *) echo "unsupported arch $ARCH"; exit 1 ;; esac
